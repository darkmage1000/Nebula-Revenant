# LightningSpell.gd - Alien Monk's starting weapon with chain lightning mechanics
extends Node2D
...
func shoot():
	...
	# Get total projectiles (base + player bonus)
	...
	var lightning_scene = preload("res://LightningProjectile.tscn")
	...
	for i in range(total_projectiles):
		...
		new_lightning.damage = final_damage
		new_lightning.global_position = shooting_point.global_position

		# Chain lightning properties
		new_lightning.chain_targets = stats.get("chain_targets", 1)
		var base_chain_range = stats.get("chain_range", 250)
		new_lightning.chain_range = base_chain_range * player.player_stats.get("attack_range_mult", 1.0)
		new_lightning.player = player

		# Evolution flags
		new_lightning.chain_storm = stats.get("chain_storm", false)
		new_lightning.thunder_strike = stats.get("thunder_strike", false)
		...
		new_lightning.rotation = direction.angle()

		player.get_parent().add_child(new_lightning)

		# Report damage for lifesteal (initial hit damage, chains will report separately)
